...
<div id="wp_wall">
		<div class="wallnav">
					<i alt="Previous" id="img_left" class="fa fa-arrow-left"></i>
					<i alt="Next" id="img_right" class="fa fa-arrow-right"></i>
					<?php if ( $show_all ) : ?>
	...
	<?php if ( ! $disable_new ) : ?>
		<?php if (  $only_registered && !$user_ID) : ?>
			...
			<a class="logtopost" href="wp-login.php"><?php _e("Log in to post a comment.", 'oblivion'); ?></a>


		<?php else : ?>

			<a id="wall_post_toggle"> <i class="fa fa-chevron-right"></i> <?php echo $wall_reply; ?></a>
	...
	<div id="wall_post">
		<form action="<?php echo get_template_directory_uri() . '/chat/wp-wall-ajax.php'; ?>" method="post" id="wallform">
			...
			<p>
			<label for="wpwall_author"><small><?php _e("Name", 'oblivion'); ?></small></label>
			<input type="text" name="wpwall_author" id="wpwall_author" value=""  tabindex="11"  />
			</p>

			<?php if ( $show_email ) : ?>
			<p>
			<label for="wpwall_email"><small><?php _e("Email", 'oblivion'); ?></small></label>
			...
			<p>
			<label for="wpwall_comment"><small><?php _e("Comment", 'oblivion'); ?></small></label>

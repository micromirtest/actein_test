...
function WPWall_Init() {
	...
	wp_register_widget_control('WP Wall', 'WPWall_WidgetControl', 'WPWall_WidgetControl');
...
function WPWall_ScriptsAction()
{
	if (!is_admin())
	{
        ...
        wp_enqueue_script('wp_wall_script',  get_template_directory_uri() . '/js/wp-wall.js', '', '',true);
	...
	if ( $comments ) {
		...
		foreach ($comments as $comment) {
			...
			if ( current_user_can( 'edit_post', $comment->comment_post_ID ) )
				$deleteurl= '<a href="javascript:verify(\''.esc_url( wp_nonce_url(get_bloginfo('wpurl')."/wp-admin/comment.php?action=deletecomment".( $options['delete_spam'] ? "&dt=spam" : "")."&p=$comment->comment_post_ID&c=$comment->comment_ID", "delete-comment_$comment->comment_ID" ) ).'\', \'\')">'.htmlspecialchars  ($comment->comment_author).'</a> <i>says:</i>';
			else
			...
			if ($author_data)
				...
				$highlight="";
			$icon = (1 == $comment->user_id) ? '<i class="fa fa-star"></i> ' : '<i class="icon-user"></i> ';
			$result.= '<p class="wall-'.$count.$oddcomment.' '.$highlight.'">'.WPWall_Gravatar($comment).'<span class="wallauthor" title="'.htmlspecialchars($comment->comment_author).' on '.$comment->comment_date.'" > '.$icon .''. $deleteurl.'</span><span class="wallcomment" title="'.htmlspecialchars(strip_tags($comment->comment_content)).'" > '.
...
function WPWall_GetOptions()
{
 ...
 $options = array(
	 'wall_title' => __('Wall', 'oblivion'),
	 'wall_reply' => __('Leave a reply', 'oblivion'),
	 'wall_comments' => 5,
	 'wall_wrap' => 25,
	 'disable_new' => '',
	 'only_registered' => '',
...
function WPWall_Options()
{
	...
	$imgpath= get_template_directory_uri() . '/chat/i';

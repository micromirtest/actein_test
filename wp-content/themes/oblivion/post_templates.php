...
class Single_Post_Template_Plugin {
	...
	function get_post_templates() {
		$templates = wp_get_theme('oblivion')->get_files( 'php', 1 );
		$post_templates = array();
		$base = array( trailingslashit( get_template_directory()), trailingslashit( get_stylesheet_directory()) );
		foreach ( (array) $templates as $file => $full_path ) {
            if( $full_path ==  get_template_directory().'/post_templates.php'){continue;}else{
	...
	function metabox( $post ) {
		...
		<label class="hidden" for="post_template"><?php  _e( 'Post Template', 'oblivion' ); ?></label><br />

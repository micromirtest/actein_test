...
add_shortcode( 'gallery', 'bootstrat_carousel_gallery_shortcode' ); /* Add custom shortcode */
function bootstrat_carousel_gallery_shortcode( $attr ) {
	...
	if ( isset( $attr['ids'] ) and isset( $attr['type'] ) and $attr['type'] == 'carousel' ) :
		...
		$attr['ids'] = $attr['ids'];
		$attr['name'] = isset( $attr['name'] ) ? sanitize_title( $attr['name'] ) : 'bootstrap-carousel' ; /* Any name. String will be sanitize to be used as HTML ID. Recomended when you want to have more than one carousel in the same page. Default: bootstrap-carousel. */
...
function bootstrat_carousel_get_html_from( $attr ) {
	/* Obtain posts array by given ids. Then construct HTML. */
	$images = bootstrat_carousel_make_array( $attr['ids'] );
	$output = '';
	if ( is_array( $images ) and !empty( $images ) ) :
		...
		if ( is_array( $posts ) and !empty( $posts ) ) :
			$output = bootstrat_carousel_make_html_from( $attr , $posts );
		endif;
	endif;
	return $output;
}
function bootstrat_carousel_make_html_from( $attr , $posts ) {
	...
	$output .= ( $attr['indicators'] == 'before-inner' ) ? bootstrat_carousel_make_indicators_html_from( $posts , $attr['name'] ) : '' ;
	/* Initialize inner. */
	...
	$output .= ( $attr['indicators'] == 'after-inner' ) ? bootstrat_carousel_make_indicators_html_from( $posts , $attr['name'] ) : '' ;
	$output .= ( $attr['control'] != 'false' ) ? bootstrat_carousel_make_control_html_with( $attr['name'] ) : '' ;
	/* Try to obtain indicators after control. */
	$output .= ( $attr['indicators'] == 'after-control' ) ? bootstrat_carousel_make_indicators_html_from( $posts , $attr['name'] ) : '' ;
	/* End carousel HTML. */
	$output .= '</div>';
...
function bootstrat_carousel_make_indicators_html_from( $posts , $name ) {
...
function bootstrat_carousel_make_control_html_with( $name ) {
	$output = '<a class="carousel-control left" href="#' . $name . '" data-slide="prev"></a>';
	$output .= '<a class="carousel-control right" href="#' . $name . '" data-slide="next"></a>';
	return $output;
}
/* Obtain array of id given comma-separated values in a string. */
function bootstrat_carousel_make_array( $string ) {

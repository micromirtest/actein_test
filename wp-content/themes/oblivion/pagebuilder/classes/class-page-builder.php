...
if(!class_exists('Page_Builder')) {
	class Page_Builder {
		...
		function __construct( $config = array()) {
			$defaults['menu_title'] = __('Page Builder', 'oblivion');
			$defaults['page_title'] = __('Page Builder', 'oblivion');
			$defaults['page_slug'] = 'page_builder';
		 ...
		 */
		function init() {
			add_action('admin_menu', array(&$this, 'oblivion_create_menu1'));
		 ...
		 * @since 1.0.0
		 */
            function oblivion_create_menu1(){
		...
		function register_template_post_type() {
			if(!post_type_exists('template')) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Page Builder notice: </strong>'. __('The \"template\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to consult with this issue', 'oblivion') .'</p></div>';"));
			}
		}
		/**
		...
		function display_blocks( $template_id ) {
			...
			if(empty($blocks)) {
				...
				echo __('Drag block items from the left into this area to begin building your template.', 'oblivion');
				echo '</p>';
				return;
			} else {
				//outputs the blocks
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				echo '<p class="empty-template">';
				echo __('This template is empty', 'oblivion');
				echo '</p>';
			} else {
				//template wrapper
		...
		function add_shortcode() {
			...
			if ( !array_key_exists( 'template', $shortcode_tags ) ) {
				...
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Page Builder notice: </strong>'. __('The \"[template]\" shortcode already exists, possibly added by the theme or other plugins. Please consult with the theme author to consult with this issue', 'oblivion') .'</p></div>';"));
			}
		}
		/**
		 * Shortcode function
		...
		function add_media_button( $button ) {
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				if ( version_compare( $wp_version, '3.5', '<' ) ) {
					$img 	= '<img src="' . get_template_directory_uri() . '/pagebuilder/assets/images/media-button.png" width="16px" height="16px" alt="' . esc_attr__( 'Add Page Template', 'oblivion' )  . '" />';
					$output = '<a href="#TB_inline?width=640&inlineId=pb-iframe-container" class="thickbox" title="' . esc_attr__( 'Add Page Template', 'oblivion' )  . '">' . $img . '</a>';
				} else {
					$img 	= '<span class="wp-media-buttons-icon" style="background-image: url(' . get_template_directory_uri() . '/pagebuilder/assets/images/media-button.png ); margin-top: -1px;"></span>';
					$output = '<a href="#TB_inline?width=640&inlineId=pb-iframe-container" class="thickbox button" title="' . esc_attr__( 'Add Page Template', 'oblivion' ) . '" style="padding-left: .4em;">' . $img . ' ' . esc_attr__( 'Add Template', 'oblivion' ) . '</a>';
				}
			}
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				<script type="text/javascript">
					function insertTemplate() {
						...
						if ( '' == id ) {
							alert("<?php echo esc_js( __( 'Please select your template first!', 'oblivion' ) ); ?>");
							return;
						}
				...
				<div id="pb-iframe-container" style="display: none;">
					<div class="wrap" style="padding: 1em">
						...
						if ( empty( $templates ) ) {
							echo sprintf( __( 'You don\'t have any template yet. Let\'s %s create %s one!', 'oblivion' ), '<a href="' .admin_url().'themes.php?page=page-builder">', '</a>' );
						}
						?>
						<h3><?php _e( 'Choose Your Page Template', 'oblivion' ); ?></h3><br />
						...
						<input type="button" id="pb-insert-template" class="button-primary" value="<?php echo esc_attr__( 'Insert Template', 'oblivion' ); ?>" onclick="insertTemplate();" />
						<a id="pb-cancel-template" class="button-secondary" onclick="tb_remove();" title="<?php echo esc_attr__( 'Cancel', 'oblivion' ); ?>"><?php echo esc_attr__( 'Cancel', 'oblivion' ); ?></a>
		...
		function contextual_help() {
			...
			if($screen->id == $this->page) {
				...
				$screen->set_help_sidebar(
                    '<p><strong>' . __('For more information:', 'oblivion') . '</strong></p>'
				);
				// Main overview tab
				$screen->add_help_tab( array(
				'id'		=> 'overview',
				'title'		=> __('Overview', 'oblivion'),

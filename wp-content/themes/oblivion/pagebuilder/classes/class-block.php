...
if(!class_exists('Block')) {
	class Block {
	 	...
	 	function pbblock($instance) {
	 		extract($instance);
	 		echo __('function Block::block should not be accessed directly. Output generated by the ', 'oblivion') . strtoupper($id_base). ' Class';
	 	}
	 	/**
		 ...
		 * Note : edited to be able to handle arrays without throwing an exception and mishandling them overall
	 	 */
		public function cleanupslashes ($entry) {
			if (is_array($entry)) {
				foreach ($entry as $key=>$value) {
					$entry[$key] = stripslashes($value);
				}
				return $entry;
			} else {
		...
		public function cleanuphtmlspecialchars ($entry) {
			if (is_array($entry)) {
				foreach ($entry as $key=>$value) {
	 	...
	 	/*
		updated by shark 20140630 2:40pm 
		array_map rewritten
		$new_instance = array_map('htmlspecialchars', array_map('stripslashes', $new_instance));
		*/
	 	function update($new_instance, $old_instance) {
	 		
	 		$new_instance = array_map(array($this, 'cleanuphtmlspecialchars'), array_map(array($this, 'cleanupslashes'), $new_instance));
			return $new_instance;
	 	}
	 	...
	 	function form($instance) {
	 		echo '<p class="no-options-block">' . __('There are no options for this block.', 'oblivion') . '</p>';
	 		return 'noform';
	 	}
	 	/**
	 	...
	 	function before_block($instance) {
	 		extract($instance);
	 		$column_class = $first ? 'first' : '';
	 		echo '<div id="block-'.$number.'" class="block block-'.$id_base.' '.$size.' '.$column_class.' cf">';
	 	}
	 	/* block footer */
	 	...
	 	function get_field_name($field) {
	 		$field_name = isset($this->block_id) ? 'blocks[' . $this->block_id. '][' . $field . ']': '';
	 		return $field_name;
	 	}
	}
}
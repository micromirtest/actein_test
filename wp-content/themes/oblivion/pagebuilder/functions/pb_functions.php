...
$output = '<input type="'.$type.'" id="'. $block_id .'_'.$field_id.'" class="input-'.$size.'" value="'.$input.'" name="blocks['.$block_id.']['.$field_id.']">';
    ...
    function field_multiselect($field_id, $block_id, $options, $selected_keys = array()) {
        ...
        foreach ($options as $key => $option) {
			...
			$output .= '<option id="'. $block_id .'_'.$field_id.'" value="'.$key.'" '. $selected .' />'.$option.'</option>';
        }
        $output .= '</select>';
        return $output;
    }
	/* Creates checkboxes from array and posts them from array field */
	function field_checkboxfromarray($field_id, $block_id, $options, $selected_keys = array()) {
		foreach ($options as $key => $option) {
            $selected = (is_array($selected_keys) && in_array($key, $selected_keys)) ? $selected = ' checked ' : '';
			$output .= '<div class="pb-cat-cont"><input id="'. $block_id .'_'.$field_id.'_'.$key.'" name="blocks['.$block_id.']['.$field_id.'][]" value="'.$key.'" '. $selected .' type="checkbox" /><label for="'. $block_id .'_'.$field_id.'_'.$key.'">'.$option.'</label></div>';
        }
		return $output;
	}
    ...
    function field_upload_for_carusel($field_id, $block_id, $media, $media_type = 'image') {

                $block_id = str_replace("_items", "",$block_id );
                $new_field_id = str_replace("[", "_",$field_id );
                $new_field_id = str_replace("]", "_",$new_field_id );
        $output = '<input type="text" id="'. $block_id .'_'.$new_field_id.'" class="input-full input-upload" value="'.$media.'" name="blocks['.$block_id.']'.$field_id.'">';
        $output .= '<a href="#" class="upload_button button" rel="'.$media_type.'">Upload</a><p></p>';
        return $output;
    }
    /**

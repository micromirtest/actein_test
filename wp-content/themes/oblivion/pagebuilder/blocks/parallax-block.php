...
class Parallax_Block extends Block {
    ...
    function __construct() {
        $block_options = array(
            'name' => __('Parallax block','oblivion'),
            'size' => __('span6','oblivion')
        );
        //create the widget
    ...
    function form($instance) {
        ...
        __('Drag block items into this parallax box', 'oblivion'),
        '</p>';
   ...
   //form footer
    ...
    function inputs($instance){
        ...
        $type_options = array(
                'light' => __('Light', 'oblivion'),
                'dark' => __('Dark', 'oblivion')
            );
        echo '<div class="fullwidth_block_wrapper">';
        echo '<div class="fullwidth_block_wrapper_title">'.__("Choose between light and dark version", 'oblivion').'</div>';
        echo field_select('type', $block_id, $type_options, $type);
        echo '<div class="fullwidth_block_wrapper_title">'.__("Choose background color", 'oblivion').'</div>';
        echo field_color_picker('color', $block_id, $color, $default = '#ffffff');
        echo '<div class="fullwidth_block_wrapper_title">'.__("Please upload image for parallax background. (Note: Video has priority over image!)", 'oblivion').'</div>';
        echo field_upload('imagebck', $block_id, $imagebck, $media_type = 'image');
        echo '<div class="fullwidth_block_wrapper_title">'.__("Video WebM Upload", 'oblivion').'</div>';
        echo '<div class="fullwidth_block_wrapper_title">'.__("Please upload webm video format. Video will be automatically played on page load.", 'oblivion').'</div>';
        echo field_upload('webm', $block_id, $webm, $media_type = 'video/webm');
        echo '<div class="fullwidth_block_wrapper_title">'.__("Video MP4 Upload", 'oblivion').'</div>';
        echo '<div class="fullwidth_block_wrapper_content">'.__("Please upload mp4 video format because of cross browser compatibility.", 'oblivion').'</div>';
        echo field_upload('mp4', $block_id, $mp4, $media_type = 'video/mp4');
        echo '<div class="fullwidth_block_wrapper_title">'.__("Preview image", 'oblivion').'</div>';
        echo '<div class="fullwidth_block_wrapper_content">'.__("Please upload preview image for video. Image will be displayed while video is loading.", 'oblivion').'</div>';
        echo field_upload('preview', $block_id, $preview, $media_type = 'image');
        echo '<div class="fullwidth_block_wrapper_title">'.__("Remove inner bottom spacing", 'oblivion').'  '.field_checkbox('marg', $block_id, $marg, $check = 'true').'</div>';
        echo '<div class="fullwidth_block_wrapper_title">'.__("Remove inner top spacing", 'oblivion').'  '.field_checkbox('margtop', $block_id, $margtop, $check = 'true').'</div>';
        echo '</div>';
    }
     function block_callback($instance) {
        ...
        if(isset($template_id)) {
            ...
            if ( !of_get_option('fullwidth') ) {
			 ...
			 if(empty($preview) && (!empty($webm) && !empty($mp4)))$preview =  get_template_directory_uri().'/img/defaults/287x222.jpg';
 echo "
        <video id=\"videop\" autoplay=\"true\" loop=\"true\" preload=\"auto\" poster=\"".$preview."\" class=\"slider-video\" style=\"left: -380px; width: 1920px;display: block;position: relative;\">

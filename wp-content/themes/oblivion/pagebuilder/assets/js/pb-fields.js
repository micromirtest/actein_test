...
//			});
	...
	function sortable_list_add_item(action_id, items) {
		...
		var newNum = maxNum + 1;
		var data = {
			action: 'block_'+action_id+'_add_new',
			security: $('#pb-nonce').val(),
			count: newNum,
			block_id: blockID
		};
	...
	$(document).on('click', 'a.sortable-add-new', function() {
		var action_id = $(this).attr('rel'),
			items = $(this).parent().children('ul.sortable-list');
		sortable_list_add_item(action_id, items);
		sortable_list_init
		return false;
	});
	// Delete Sortable Item
	$(document).on('click', '.sortable-list a.sortable-delete', function() {
	...
	$(document).on('click', '.sortable-list .sortable-handle a', function() {
		var $clicked = $(this);
		$clicked.addClass('sortable-clicked');
		$clicked.parents('.sortable-list').find('.sortable-body').each(function(i, el) {
